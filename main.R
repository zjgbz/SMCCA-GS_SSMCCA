...
UpdateW_nores <- function(xlist, i, K, sumabsthis, ws, type="standard", ws.final){
	tots <- 0
	for(j in (1:K)[-i]){
		diagmat <- (t(ws.final[[i]])%*%t(xlist[[i]]))%*%(xlist[[j]]%*%ws.final[[j]])
		diagmat[row(diagmat)!=col(diagmat)] <- 0
...
# PMA not update penalty with res
MultiCCA_nores.permute <- function(xlist, penalties=NULL, ws=NULL, type="standard", nperms=10, niter=3, trace=TRUE, standardize=TRUE){
	call <- match.call()
	K <- length(xlist)
	for(k in 1:K){
	...
	for(i in 1:ncol(penalties)){
				out <- MultiCCA_nores(xlist, penalty=penalties[,i], niter=niter, type=type, ws=ws, trace=trace)
	...
	for(j in 1:nperms){
		...
		for(i in 1:ncol(penalties)){
						out <- MultiCCA_nores(xlistperm, penalty=penalties[,i], niter=niter, type=type, ws=ws, trace=FALSE)
...
MultiCCA_nores <- function(xlist, penalty=NULL, ws=NULL, niter=25, type="standard", ncomponents=1, trace=TRUE, standardize=TRUE){
	...
	for(comp in 1:ncomponents){
		...
		while(curiter<=niter && abs(crit.old-crit)/abs(crit.old)>.001 && crit.old!=0){
			...
			curiter <- curiter+1
			for(i in 1:K){
				ws[[i]] <- UpdateW_nores(xlist, i, K, penalty[i], ws, type[i], ws.final)
			}
		}
...
# PMA update penalty without res
MultiCCA_nores_1st.permute <- function(xlist, penalties=NULL, ws=NULL, type="standard", nperms=10, niter=3, trace=TRUE, standardize=TRUE){
	call <- match.call()
	K <- length(xlist)
	for(k in 1:K){
			if(ncol(xlist[[k]])<2) stop("Need at least 2 features in each data set!")
	...
	cors <- numeric(ncol(penalties))
	for(i in 1:ncol(penalties)){
				out <- MultiCCA_nores_1st(xlist, penalty=penalties[,i], niter=niter, type=type, ws=ws, trace=trace)
				cors[i] <- GetCors(xlist, out$ws, K)
				numnonzeros <- c(numnonzeros, sum(out$numnonzeros))
				ws.init  <- out$ws.init
	}
	...
	for(j in 1:nperms){
		...
		for(i in 1:ncol(penalties)){
						out <- MultiCCA_nores_1st(xlistperm, penalty=penalties[,i], niter=niter, type=type, ws=ws, trace=FALSE)
						permcors[j,i] <- GetCors(xlistperm, out$ws, K)
		}
	}
	pvals =zs =  NULL
	...
	if(trace) cat(fill=TRUE)
	out <- list(pvals=pvals, zstat=zs, bestpenalties=penalties[,which.max(zs)], cors=cors, corperms=permcors, numnonzeros=numnonzeros, ws.init=ws.init, call=call, penalties=penalties, type=type, nperms=nperms)
	class(out) <- "MultiCCA_nores_1st.permute"
	return(out)
}

MultiCCA_nores_1st <- function(xlist, penalty=NULL, ws=NULL, niter=25, type="standard", ncomponents=1, trace=TRUE, standardize=TRUE){
	for(i in 1:length(xlist)){
		if(ncol(xlist[[i]])<2) stop("Need at least 2 features in each data set.")
	}
	call <- match.call()
	...
	for(comp in 1:ncomponents){
		ws <- list()
		for(i in 1:length(ws.init)) ws[[i]] <- ws.init[[i]]
		curiter <- 1
		crit.old <- -10
		crit <- -20
		storecrits <- NULL
	...
	out <- list(ws=ws.final, ws.init=ws.init, K=K, call=call, type=type, penalty=penalty, cors=cors)
	class(out) <- "MultiCCA_nores_1st"
	return(out)
}

MultiCCA_nores_update_penalty <- function(xlist, penalty=NULL, ws=NULL, niter=25, nperms=10, perm_niter=3, type="standard", ncomponents=1, trace=TRUE, standardize=TRUE){
	for(i in 1:length(xlist)){
		if(ncol(xlist[[i]])<2) stop("Need at least 2 features in each data set.")
	}
	call <- match.call()
	...
	penalty = matrix(NA, nrow=ncomponents, ncol=K)
	ws.init <- ws
	ws.final <- list()
	for(i in 1:length(ws)) ws.final[[i]] <- matrix(0, nrow=ncol(xlist[[i]]), ncol=ncomponents)
	cors <- NULL
	for(comp in 1:ncomponents){
		...
		perm_out = MultiCCA_nores_1st.permute(xlist, penalties=NULL, ws=ws, type="standard", nperms=nperms, niter=perm_niter, trace=TRUE, standardize=TRUE)
		penalty[comp, ] = perm_out$bestpenalties
		...
		while(curiter<=niter && abs(crit.old-crit)/abs(crit.old)>.001 && crit.old!=0){
			...
			curiter <- curiter+1
			for(i in 1:K){
				ws[[i]] <- UpdateW_nores(xlist, i, K, penalty[comp, i], ws, type[i], ws.final)
			}
		}
		for(i in 1:length(ws)) ws.final[[i]][,comp] <- ws[[i]]
		cors <- c(cors, GetCors(xlist, ws,K))
	...
	out <- list(ws=ws.final, ws.init=ws.init, K=K, call=call, type=type, penalty=penalty, cors=cors)
	class(out) <- "MultiCCA_nores_update_penalty"
	return(out)
}

# PMA update penalty with res
MultiCCA_1st.permute <- function(xlist, penalties=NULL, ws=NULL, type="standard", nperms=10, niter=3, trace=TRUE, standardize=TRUE){
	call <- match.call()
	K <- length(xlist)
	for(k in 1:K){
			if(ncol(xlist[[k]])<2) stop("Need at least 2 features in each data set!")
	...
	cors <- numeric(ncol(penalties))
	for(i in 1:ncol(penalties)){
				out <- MultiCCA_1st(xlist, penalty=penalties[,i], niter=niter, type=type, ws=ws, trace=trace)
				cors[i] <- GetCors(xlist, out$ws, K)
				numnonzeros <- c(numnonzeros, sum(out$numnonzeros))
				ws.init  <- out$ws.init
	}
	...
	for(j in 1:nperms){
		...
		for(i in 1:ncol(penalties)){
						out <- MultiCCA_1st(xlistperm, penalty=penalties[,i], niter=niter, type=type, ws=ws, trace=FALSE)
						permcors[j,i] <- GetCors(xlistperm, out$ws, K)
		}
	}
	pvals =zs =  NULL
...
MultiCCA_1st <- function(xlist, penalty=NULL, ws=NULL, niter=25, type="standard", ncomponents=1, trace=TRUE, standardize=TRUE){
	for(i in 1:length(xlist)){
		if(ncol(xlist[[i]])<2) stop("Need at least 2 features in each data set.")
	}
	call <- match.call()
...
MultiCCA_update_penalty <- function(xlist, penalty=NULL, ws=NULL, niter=25, nperms=10, perm_niter=3, type="standard", ncomponents=1, trace=TRUE, standardize=TRUE){
	for(i in 1:length(xlist)){
		if(ncol(xlist[[i]])<2) stop("Need at least 2 features in each data set.")
	}
	call <- match.call()
	...
	for(comp in 1:ncomponents){
		...
		perm_out = MultiCCA_1st.permute(xlist, penalties=NULL, ws=ws, type="standard", nperms=nperms, niter=perm_niter, trace=TRUE, standardize=TRUE)
		penalty[comp, ] = perm_out$bestpenalties
		...
		while(curiter<=niter && abs(crit.old-crit)/abs(crit.old)>.001 && crit.old!=0){
			...
			curiter <- curiter+1
			for(i in 1:K){
				ws[[i]] <- UpdateW(xlist, i, K, penalty[comp, i], ws, type[i], ws.final)
			}
		}
		for(i in 1:length(ws)) ws.final[[i]][,comp] <- ws[[i]]
		cors <- c(cors, GetCors(xlist, ws,K))
...
# GS not update penalty
MultiCCA_rmeffect_nores <- function(moData, penalty, ncomponents = 1, nperms = 10, niter = 25) {
	...
	for (comp_i in 1:ncomponents) {
		print(comp_i)
		fullMCCA2 = MultiCCA_nores(moData_scale, penalty, ncomponents = 1, niter = niter)
	...
	out = list(canon_var = cv_list, weight = weight_list)
	return (out)
}