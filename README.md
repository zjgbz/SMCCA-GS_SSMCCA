...
| niter       | (optional) numerical value for the number of iterations used to update CV weights. Note that CV weights updating would stop early if sum correlation across all CV pairs converges before reach the ```niter```th interation. The default value is ```niter=25```.                                                                                                                    |
